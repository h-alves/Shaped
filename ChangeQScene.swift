...
class ChangeQScene: SKScene, SKPhysicsContactDelegate{
    ...
    override func touchesBegan(_ touches: Set<UITouch>, with event: UIEvent?) {
        ...
        if arty.contains(location) {
            ...
            if(contagem >= 4 && contagem < 8) {
                arty.setScale(1.0)
                arty.size = CGSize(width: frame.width * 0.26, height: frame.height * 0.35)
                arty.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame) + self.frame.height * 0.02)
                arty.texture = SKTexture(imageNamed: "abstractQ1")
            }else if(contagem >= 8 && contagem < 16) {
                arty.setScale(1.0)
                arty.size = CGSize(width: frame.width * 0.24, height: frame.height * 0.35)
                arty.texture = SKTexture(imageNamed: "abstractQ2")
            }else if(contagem >= 16) {
                arty.setScale(1.0)
                arty.size = CGSize(width: frame.width * 0.24, height: frame.height * 0.32)
                arty.position = CGPointMake(CGRectGetMidX(self.frame), CGRectGetMidY(self.frame))
                arty.texture = SKTexture(imageNamed: "abstractQ3")
                arty.run(.sequence([
                    .wait(forDuration: 1)
                ]),completion: {
    ...
    func changeScene(){
        gameManager?.goToScene(.animationSquareScene)
    }
}
